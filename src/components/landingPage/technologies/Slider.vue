...
let halfVisibleObserver: IntersectionObserver
...
function createFullyHiddenObserver(callback: IntersectionObserverCallback): void{
    ...
    console.log("Element to observe is: ", slider.value!.children[1]);
    fullyHiddenObserver.observe(slider.value!.children[1])
}

function loopFirstChildCallback(thresholds: IntersectionObserverEntry[], observer: IntersectionObserver){
    console.log(thresholds);

    if (thresholds[0].intersectionRatio >= 0.48)

    return
    // The first child is visible
    if (thresholds[0].isIntersecting) return
    fullyHiddenObserver.unobserve(slider.value!.children[1])
...
<template>
    <div class="bg-secondary px-10 py-20">
        <div class="flex overflow-x-scroll space-x-4" ref="slider">
            ...
            <div class="flex-center  h-64 w-64 shrink-0 bg-green-200" v-for="(string, i) of strings" :key="string" :style="{ opacity: i / 10}">
                {{ string }}
            </div>
        </div>
    </div>
